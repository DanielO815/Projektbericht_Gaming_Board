...
app.get('/getFilteredRows', async (req, res) => {
  const { table, filterCol, filterVal } = req.query;

  if (!table || !filterCol || !filterVal) {
    return res.status(400).json({ error: 'table, filterCol, und filterVal sind erforderlich' });
  }

  const queryText = `SELECT * FROM dbo.${table} WHERE ${filterCol} = @filterVal`;
